...
class CWebUser extends CApplicationComponent implements IWebUser
{
	...
	/**
	 * Created by Sasanka on 11/May/2013
	 * Performs to get the permission according to user.	
	 * @return the boolean value if allowed send true.
	 * To get the role call as Yii::app()->user->GetPermission($ControllerName,$ActionName);
	 */
	public function GetPermission($ControllerName,$ActionName)
	{
		$ControllerName=strtolower($ControllerName);
		$ActionName=strtolower($ActionName);

		$userArr = Yii::app()->db->createCommand()
                ->select('Role_ID, superuser')
                ->from('tbl_users')
                ...
                if(count($userArr) > 0)
                {
                    $role = $userArr[0]['Role_ID'];
                    $superuser = $userArr[0]['superuser'];
                }
                
                

        $controllerID=Yii::app()->db->createCommand()
                ->select('ID')
        ...
        $actionID=Yii::app()->db->createCommand()
                ...
                ->where('action_name="'.$ActionName.'" and controller_Id="'.$controllerID.'"')
                ->queryScalar();
        ...
        $access = Yii::app()->db->createCommand()
                ->select('ID')
                ->from('access_user_roll')
                ->where('Role_ID="'.$role. '" and Contoller_ID="'.$controllerID. '" and action_id="'.$actionID.'"')
                ->queryScalar();                
