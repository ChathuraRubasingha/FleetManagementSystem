...
class FuelConsumptionByVehecleDateForm extends CFormModel
{
   ...
   public function rprint($Vehicle_No, $vFromDate, $vToDate)
	{
		...
		$mystr=$DataSource->ArrayFuelConsumptionbyVehicle($Vehicle_No, $vFromDate, $vToDate);
		
		$mystrSummary=$DataSource->ArrayFuelConsumptionbyVehicleInSummary($Vehicle_No, $vFromDate, $vToDate);
		...
		//$pdf->ezText('Local Governance Program',15);
		 ...
		 $pdf->ezText('',5);
		 $pdf->ezText('Fuel Consumption Report - (From: '.$vFromDate.' To: '.$vToDate.')',10);
		 $pdf->ezText('',5);
		...
		{
		 $pdf->ezTable($mystr,'','',
		 array('cols' => array('Date' => array('width'=>'80'), 'Volumn(L)     (a)' => array('width'=>'67'), 'Amount(Rs)      (b)' => array('justification' => 'right', 'width'=>'100'), 'Starting Odometer      (c)' => array('width'=>'87'), 'Distance Driven(km)        (d)' => array('width'=>'87'), 'Liters per 100km(L/100km)    (a*100)/d' => array('width'=>'100'))));

		 $pdf->ezText('',15);
		 $pdf->ezText('<b>Summary details in total</b>',12);
		 $pdf->ezText('',10);
		 $pdf->ezTable($mystrSummary,'','',
		 array('cols' => array('Total Volume(L)' => array('width'=>'100'), 'Total Amount(Rs)' => array('justification' => 'right', 'width'=>'150'), 'Total Distance Driven(km)' => array('width'=>'130'), 'Total LPKM' => array('width'=>'150'))));
		}
		else
		{
